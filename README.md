...
## Deployment (Freebuff + Convex Cloud)

The app is split into two parts: the **frontend** (hosted on Freebuff)
and the **Convex backend** (functions + database, Convex Cloud).

### Frontend → Freebuff

Freebuff hosts the Vite frontend automatically — just push your changes
from the Freebuff editor. No extra build or deploy steps needed.

### Backend → Convex Cloud

Convex functions are deployed by Freebuff when you edit Convex source files.
To deploy manually or via CI:

```bash
bunx convex deploy
```

Set **server-side** env vars in the **Convex dashboard** (read via
`process.env` inside actions):

| Variable | Purpose |
|---|---|
| `LLM_API_KEY` | Key for any OpenAI-compatible LLM endpoint (e.g. OpenRouter) |
